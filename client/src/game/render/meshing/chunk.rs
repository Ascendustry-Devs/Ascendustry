        ...
        for d in 0..=CHUNK_SIZE {
            ...
            for u in 0..=LAST_CHUNK_AXIS_INDEX {
                ...
                for v in 0..=LAST_CHUNK_AXIS_INDEX {
                    ...
                    let previous_is_solid = solidity[((previous_pos[0] + 1)
                        + (previous_pos[1] + 1) * PADDED_CHUNK_SIZE
                        + (previous_pos[2] + 1) * PADDED_CHUNK_SIZE_SQR) as usize];
                    let current_is_solid = solidity[((current_pos[0] + 1)
                        + (current_pos[1] + 1) * PADDED_CHUNK_SIZE
                        + (current_pos[2] + 1) * PADDED_CHUNK_SIZE_SQR) as usize];
